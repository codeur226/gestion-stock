        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Gestion des caracteristique de "{{ optional($selectedCategorieMateriel)->nom }}" </h5>
                ...
                <div class="modal-body">
                   ...
                   <table class="table table-bordered">
                        ...
                        <tbody>
                            @forelse ($proprietesMateriels as $prop)
                                <tr>    
                                    <td>{{ $prop->nom }}</td>
                                    <td>{{ $prop->estObligatoire == 0 ? "NON": "OUI" }}</td>
                                    <td>
                                        <button class="btn btn-link" data-toggle="modal" data-target="#editModalProp" wire:click="editProp({{$prop->id}})"> <i class="far fa-edit"></i> </button>
                                        @if(count($prop->materiels)==0)
                            ...
                            @empty
                                <tr>
                                    <td colspan="3">
                                        <span class="text-info">Vous n'avez pas encore des propriétés définies pour ce categorie de materiel</span>
                                    </td>
                                </tr>
                            @endforelse
